...
userSchema.statics = {
  ...
  findUserAndCreateNumberVerify(email) {
    return this.findOneAndUpdate(
      ...
      { "local.verifyNumber": Math.round(Math.random() * 1000000) },
      { new: true }
    ).exec();
  },
  findUserByEmailAndVerifyNumber(email, verifyNumber) {
    return this.findOne({
      "local.email": email,
      "local.verifyNumber": verifyNumber,
    }).exec();
  },
  FindUserByEmailAndUpdateNewPassword(email, hashPassword) {
    return this.findOneAndUpdate(
      { "local.email": email },
      { "local.password": hashPassword, "local.verifyNumber": null },
      { new: true }
    ).exec();
  },
  updateUserAvatar(userId, userUpdateItem) {
    return this.findByIdAndUpdate(userId, userUpdateItem, {
      "local.password": 0,
      role: 0,
      isBlocked: 0,
      "local.isActive": 0,
  ...
  findUserWithDeprecatedUsersIdAndSkipNumber(
    deprecatedUsersId,
    searchKey,
    skipNumber
  ) {
  ...
  updateUserHasBeenOffline(userId) {
    return this.findByIdAndUpdate(
      userId,
      { "otherProps.lastTimeOnline": Date.now() },
      { new: true }
    ).exec();
  },
};
